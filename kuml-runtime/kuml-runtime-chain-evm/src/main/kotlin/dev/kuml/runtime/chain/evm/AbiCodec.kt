...
internal object AbiCodec {
    ...
    fun decodeBytes32(hexResult: String): ByteArray {
        ...
        if (bytes.size < 32) {
            throw EvmChainAdapterException.MalformedResponse(
                message = "Expected 32-byte ABI result, got ${bytes.size} bytes",
            )
        }
    ...
    fun decodeUint(hexResult: String): Int {
        ...
        if (bytes.size < 32) {
            throw EvmChainAdapterException.MalformedResponse(
                message = "Expected 32-byte ABI result for uint, got ${bytes.size} bytes",
            )
        }
    ...
    fun decodeString(hexResult: String): String {
        ...
        if (bytes.size < 64) {
            throw EvmChainAdapterException.MalformedResponse(
                message = "ABI string result too short: ${bytes.size} bytes",
            )
        }
        ...
        val offset = readUint32AsLong(bytes = offsetBytes, byteOffset = 28)
        if (offset < 32L || offset + 32L > bytes.size.toLong()) {
            throw EvmChainAdapterException.MalformedResponse(
                message = "ABI string offset $offset out of range for result of ${bytes.size} bytes",
            )
        }
        ...
        val length = readUint32AsLong(bytes = lengthBytes, byteOffset = 28)
        if (length > MAX_STRING_BYTES) {
            throw EvmChainAdapterException.MalformedResponse(
                message = "ABI string length $length exceeds maximum allowed $MAX_STRING_BYTES bytes",
            )
        }
        val dataStart = offset + 32L
        if (dataStart + length > bytes.size.toLong()) {
            throw EvmChainAdapterException.MalformedResponse(
                message = "ABI string length $length extends beyond result buffer",
            )
        }
