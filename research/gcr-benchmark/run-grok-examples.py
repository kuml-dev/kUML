#!/usr/bin/env python3
"""Run the GCR benchmark corpus's kUML cells against xAI's Grok, with the
real `kuml.examples` MCP tool available via Grok's OpenAI-compatible
function-calling. Companion to run-grok.py (the "without kuml.examples"
baseline) and the other kuml.examples arms (run-claude-examples.py,
run-gemini-examples.py, run-gpt4o-examples.py, run-ollama-examples.py) --
...
Mechanics: identical wire format to run-gpt4o-examples.py (xAI's
`/v1/chat/completions` tool-calling shape is OpenAI-compatible), pointed at
api.x.ai instead of api.openai.com. Whether Grok reliably calls the tool
when offered (vs. ignoring it, or -- per the llama4:scout precedent in this
benchmark -- degrading generation quality just from the tool being present)
is itself an open question this script answers empirically; don't assume
either outcome going in.

Usage:
    export XAI_API_KEY=...
    export KUML_MCP_BIN=/path/to/kuml-mcp/build/install/kuml-mcp/bin/kuml-mcp
    python3 run-grok-examples.py --model grok-4 [--workers 3] [--limit N]
"""
import argparse
...
MAX_TOOL_ROUNDS = 4  # mirrors run-gpt4o-examples.py / run-ollama-examples.py
...
# --------------------------------------------------------------------------
# xAI Chat Completions API, tool-use loop
# --------------------------------------------------------------------------
...
def call_grok_with_examples(model, prompt):
...
def run_kuml_only_benchmark(model, out_path, workers=3, limit=None, ids=None):
    ...
    gcr_loop = make_gcr_loop(call_grok_with_examples, QuotaExceeded, quota_event)
