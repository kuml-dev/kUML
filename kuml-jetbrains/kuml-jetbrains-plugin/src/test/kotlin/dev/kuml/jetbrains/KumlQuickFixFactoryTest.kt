...
class KumlQuickFixFactoryTest :
    StringSpec({
        ...
        "quickFixFor returns AddMissingParameterFix for missing parameter" {
            val diag = KumlDiagnostic("No value passed for parameter 'name'", 1, 1, 1, 1, KumlDiagnostic.Severity.ERROR)
        ...
        "quickFixFor returns RemoveUnknownParameterFix for unknown parameter" {
            val diag = KumlDiagnostic("No parameter with name 'returnType' found", 1, 1, 1, 1, KumlDiagnostic.Severity.ERROR)
            val fix = KumlQuickFixFactory.quickFixFor(diag)
            fix shouldNotBe null
        ...
        "quickFixFor returns SuppressWarningFix for warnings" {
            val diag = KumlDiagnostic("Some deprecation warning", 1, 1, 1, 1, KumlDiagnostic.Severity.WARNING)
            val fix = KumlQuickFixFactory.quickFixFor(diag)
            fix shouldNotBe null
        ...
        "quickFixFor returns null for unrecognised error" {
            val diag = KumlDiagnostic("Some random error", 1, 1, 1, 1, KumlDiagnostic.Severity.ERROR)
            val fix = KumlQuickFixFactory.quickFixFor(diag)
            fix shouldBe null
        }

        "RenameToKnownFunctionFix is available for known typos" {
            val diag = KumlDiagnostic("Unresolved reference: assoc", 1, 1, 1, 1, KumlDiagnostic.Severity.ERROR)
            val fix = KumlQuickFixFactory.quickFixFor(diag)
            fix shouldNotBe null
        ...
        "RenameToKnownFunctionFix is null for unknown reference" {
            val diag = KumlDiagnostic("Unresolved reference: fooBarBaz", 1, 1, 1, 1, KumlDiagnostic.Severity.ERROR)
            val fix = KumlQuickFixFactory.quickFixFor(diag)
