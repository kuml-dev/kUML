...
class ErmMartinSvgTest :
    StringSpec({
        ...
        "notation override BACHMAN/CHEN/IDEF1X throws a structured not-yet-supported error" {
            val customer = ErmEntity(id = "customer", name = "Customer", attributes = listOf(pk("id")))
            val model = ErmModel(name = "Shop", entities = listOf(customer))
            ...
            listOf(ErmNotation.BACHMAN, ErmNotation.CHEN, ErmNotation.IDEF1X).forEach { notation ->
                val ex =
