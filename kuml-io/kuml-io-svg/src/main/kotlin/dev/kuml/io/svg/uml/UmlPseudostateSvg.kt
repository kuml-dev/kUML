    ...
    builder.tag("g", mapOf("id" to xmlEscapeAttr(element.id))) {
        when (element.kind) {
            PseudostateKind.INITIAL -> {
                ...
                tag("circle", mapOf(
                    "cx" to fmt(cx), "cy" to fmt(cy), "r" to fmt(r),
            ...
            PseudostateKind.FORK, PseudostateKind.JOIN -> {
                tag("rect", mapOf(
                    "x" to fmt(x), "y" to fmt(cy - 4f),
                    "width" to fmt(w), "height" to "8",
                    "class" to "kuml-class", "fill" to "currentColor",
                ))
            }
            else -> {
                ...
                tag("rect", mapOf(
                    "x" to fmt(x), "y" to fmt(y),
                    "width" to fmt(w), "height" to fmt(h),
                    "class" to "kuml-class",
                ))
                tag("text", mapOf(
                    "class" to "kuml-small",
                    "x" to fmt(cx), "y" to fmt(cy + 3f),
