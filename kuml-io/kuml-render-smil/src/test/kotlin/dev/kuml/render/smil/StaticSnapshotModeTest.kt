...
class StaticSnapshotModeTest :
    StringSpec({
        ...
        fun timelineWithFill() =
            SmilTimeline(
                listOf(SmilAnimation.Fill(elementId = "n1", color = "#ffd54a", beginMs = 0, durationMs = 600)),
            )
        ...
        "STRIPPED mode removes all animate tags from svg" {
            ...
            val result = emitter.inject(svg = svg, timeline = timelineWithFill(), staticSnapshot = StaticSnapshotMode.STRIPPED)
        ...
        "STRIPPED mode removes animateTransform and set tags" {
            ...
            val result = emitter.inject(svg = svg, timeline = SmilTimeline.EMPTY, staticSnapshot = StaticSnapshotMode.STRIPPED)
            result shouldNotContain "<animateTransform"
        ...
        "STRIPPED mode on already-clean svg is a no-op idempotent" {
            ...
            val result = emitter.inject(svg = cleanSvg, timeline = SmilTimeline.EMPTY, staticSnapshot = StaticSnapshotMode.STRIPPED)
            result shouldBe cleanSvg
        }

        "ANIMATED mode adds SMIL elements" {
            val svg = "<svg><rect id=\"n1\"/></svg>"
            val result = emitter.inject(svg = svg, timeline = timelineWithFill(), staticSnapshot = StaticSnapshotMode.ANIMATED)
