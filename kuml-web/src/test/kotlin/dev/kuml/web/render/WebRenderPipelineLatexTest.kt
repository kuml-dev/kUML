...
class WebRenderPipelineLatexTest :
    FunSpec({
        ...
        test("latex format returns TikZ snippet for UML class diagram") {
            val result = WebRenderPipeline.render(script = umlClassScript, format = "latex", themeName = null, layoutOverride = null)
        ...
        test("latex format returns TikZ for SysML2 BDD diagram") {
            val result = WebRenderPipeline.render(script = sysml2BddScript, format = "latex", themeName = null, layoutOverride = null)
            val tex = result.shouldBeInstanceOf<WebRenderResult.Latex>().tex
            tex.shouldNotBeEmpty()
            tex shouldContain "tikzpicture"
        }
        ...
        test("unknown format returns WebRenderResult.Error") {
            val result = WebRenderPipeline.render(script = umlClassScript, format = "xml", themeName = null, layoutOverride = null)
