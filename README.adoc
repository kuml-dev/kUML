...
| *Docker*
| `docker pull ghcr.io/kuml-dev/kuml-cli:<version>` — minimal CLI image (V2.0.32).

| *Native installers*
| DEB, RPM, DMG, MSI packages built by the `release-installers.yml` CI workflow on every version tag (V2.0.32). Download from the GitHub Releases page.

| *Maven Central* (library modules)
| `implementation("dev.kuml:kuml-core-dsl:0.6.0")` — library modules published to Maven Central.
|===
...
| Grid-layout default (class/component) | ✅ | ❌ | ❌ | ⚠️ | ❌ | ⚠️ | ⚠️ | ❌
...
| Override the per-diagram layout engine. `grid` is now the default for Class, Component, UseCase, and State diagrams; `elk` uses the ELK Sugiyama layered engine. Added in V2.0.26.
...
`kuml render --layout=grid\|elk\|auto` (grid default) ✅ +
JetBrains plugin (annotator, quick fixes, live SVG preview, structure view) ✅ +
...
| ✅ v0.6.0 released — V2.0.22–V2.0.33
|===
