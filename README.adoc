...
*kUML* is a modelling tool that expresses UML 2.x, SysML 2, C4, BPMN 2.0 and ERM as a
type-safe Kotlin DSL — the first UML tool deliberately designed for the LLM era.

image:https://img.shields.io/maven-central/v/dev.kuml/kuml-core-dsl[Maven Central,link=https://central.sonatype.com/artifact/dev.kuml/kuml-core-dsl]
image:https://img.shields.io/jetbrains/plugin/v/dev.kuml.ide?label=JetBrains%20Plugin&logo=intellijidea[JetBrains Marketplace,link=https://plugins.jetbrains.com/plugin/dev.kuml.ide]
image:https://img.shields.io/github/actions/workflow/status/kuml-dev/kUML/ci.yml?branch=master&label=CI&logo=github[CI,link=https://github.com/kuml-dev/kUML/actions/workflows/ci.yml]
image:https://img.shields.io/github/actions/workflow/status/kuml-dev/kUML/codeql.yml?branch=master&label=CodeQL&logo=github[CodeQL,link=https://github.com/kuml-dev/kUML/actions/workflows/codeql.yml]
...
image:https://img.shields.io/github/license/kuml-dev/kUML[Apache 2.0,link=https://github.com/kuml-dev/kUML/blob/master/LICENSE]
...
Every feature in kUML exists in _some_ tool on the market. The specific combination does not exist anywhere else: *type-safe Kotlin DSL · UML 2.x + SysML 2 + C4 + BPMN 2.0 + ERM as first-class peers · executable behaviour runtime · LLM-first design · Apache 2.0.*
...
| *Docker*
| `docker pull ghcr.io/kuml-dev/kuml-cli:v0.43.0` — minimal CLI image.

| *Native installers*
| DEB, RPM, DMG, MSI packages built by the `release-installers.yml` CI workflow on every version tag (v0.43.0). Download from the GitHub Releases page.

| *Maven Central* (library modules)
| `implementation("dev.kuml:kuml-core-dsl:0.43.0")` — library modules published to Maven Central.
|===
...
| *Martin (crow's-foot)*
| The default and most widely recognised ERD notation — cardinality glyphs on the connecting line.

| *Bachman*
| Directed-arrow relationships, common in older CASE tools and database-design textbooks.

| *Chen*
| The original 1976 ER notation — attributes as ovals, relationships as their own diamond-shaped nodes.

| *IDEF1X*
| Solid lines for identifying relationships, dashed for non-identifying, rounded corners for dependent (weak) entities, plus subtype/supertype category clusters.
|===

An ERM model can be authored directly (`ermModel { … }`) or derived from an existing UML class diagram (`kuml transform --transformer uml-to-erm-script`). From there, `kuml generate --plugin sql` produces SQL DDL (PostgreSQL, MySQL, SQLite) and `kuml generate --plugin exposed` produces Kotlin Exposed tables — both consuming the ERM model, not the UML model directly, so mapping decisions (inheritance strategy, M:N junction tables, FK naming) stay inspectable. `kuml reverse --format sql` runs the chain in reverse, reconstructing an ERM model from existing SQL DDL. The SQL/Exposed generators also recognize PostGIS geometry columns (`ErmDataType.Custom("geometry(Point,4326)")`) and TimescaleDB hypertables (`hypertable(timeColumn, chunkInterval?)`) on Postgres, and `kuml generate --sql-migration --from old.kuml.kts --to new.kuml.kts` emits an additive-only Flyway schema-diff migration (refusing on any destructive change). Full reference: link:docs/handbook/modules/reference/pages/erm-dsl.adoc[ERM DSL].
...
| ERM / DB modelling (4 notations) | ✅ | ⚠️ | ⚠️ | ⚠️ | ❌ | ⚠️ | ⚠️ | ❌
| Type-safe DSL                | ✅ | ❌ | ❌ | ❌ | ⚠️ | ❌ | ❌ | ⚠️
| Real metamodel               | ✅ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ❌
| OCL constraints              | ✅ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ❌
...
| `kuml workspace init\|info\|validate\|render\|convert`
| Treat an Obsidian-vault-style directory of Markdown files with YAML frontmatter and embedded ` ```kuml ` blocks as an Open Knowledge Format (OKF) workspace (ADR-0011): `init` scaffolds a new workspace (`--name`, `--mode knowledge\|engineering`, `--output`), `info <dir>` summarises documents by type, `validate <dir>` checks frontmatter/structure, `render <dir>` renders the embedded diagrams to SVG, `convert <src> --to okf\|kts` (v0.38.0) wraps/unwraps a workspace note ⇔ a bare `.kuml.kts` script.
...
| Execute a simulation trace against a UML, SysML 2, or BPMN 2.0 behavioural model. Supports STM (state-machine semantics) for UML and SysML 2, and token-flow activity semantics (UML Activity, SysML 2 ACT, and BPMN Process — exclusive/parallel/inclusive gateways, sub-processes, terminate/flow-final) via the shared `TokenFlowEngine`. SysML 2 STM added in V2.0.17; SysML 2 ACT added in V2.0.18; BPMN Process added in ADR-0015 (`--process <id>` selects among multiple declared processes, `--max-tokens`/`--time-budget-ms` bound token-flow execution).
...
| `kuml ai audit [--output text\|json]`
| Read the compliance audit log (`~/.kuml/audit/`) of every LLM call — IDs and timestamps only, never prompts or model responses. (V3.1.x)

| `kuml ai vault unlock\|lock\|status`
| Manage the API-key vault's master-password encryption: derive the key and re-encrypt stored keys (`unlock`), zero-fill the in-memory key (`lock`), or show the current backend/encryption state (`status`). (V3.1.20)

| `kuml dump-json <script> --diagram-out <f> --layout-out <f> [--model-out <f>]`
| Dump the parsed diagram, its owning model, and the computed `LayoutResult` as JSON — the prerequisite input for the wasm playground render path. Supports UML, C4, SysML 2, BPMN, Blueprint.

| `kuml asciidoc --input <f> --output <f>` / `--input-dir <d> --output-dir <d>`
| Pre-render `[source,kuml]` blocks and `kuml::path[]` macros in AsciiDoc files to SVG/PNG, producing plain AsciiDoc that Antora's Node.js Asciidoctor pipeline can consume directly (Antora cannot load the JVM `kuml-asciidoc` extension itself). Directory mode mirrors an entire tree, copying non-`.adoc` files through unchanged.

| `kuml trace replay <trace> <script> [--verbose]` / `kuml trace export [--format otlp-json]`
| Inspect, replay, and export behaviour traces (V2.0.39).

| `kuml sandbox validate <script> [--strict] [--guard-timeout-ms N]`
| Validate guards/actions against the sandbox policy (V2.0.40).

| `kuml chain connect\|verify\|events\|sign\|verify-sig`
| Connect a behaviour-runtime state machine to a live on-chain contract (EVM by default, pluggable adapter), verify the on-chain `modelHash`, stream events, and sign/verify off-chain payloads. (V3.0.5)
...
`kuml-vault-examples-tests` — 47 CI smoke tests rendering all vault examples ✅
| ✅ Done (`v0.12.0`)
...
kuml-mcp/              ✅  MCP server — stdio JSON-RPC 2.0
                           Authoring tools (6): kuml.render / validate / list_elements / describe / generate / examples
...
kuml-web/              ✅  Browser-based editing environment (V2.0.34)
                           ...
                           Drag-and-drop node placement for class diagrams (v0.29.0)
                           ERM diagrams end-to-end, all four notations (v0.29.0)

kuml-widget/
  kuml-widget-compose  ✅  Executable behaviour widget (Compose Multiplatform, V2.0.43)
                           OclGuardEditor: interactive, syntax-highlighted OCL guard
                           editing with protected-transition confirmation gate (v0.29.0)
...
kuml-lang-support/     ✅  Editor-agnostic completion/rename/diagnostics/CLI-locator
                           logic, shared by kuml-jetbrains and kuml-language-server (v0.29.0)

kuml-language-server/  ✅  LSP4J-based Language Server: diagnostics via CLI subprocess,
                           full 38-item DSL completion catalogue (v0.29.0)
                           Consumed by kuml-jetbrains and by the VS Code extension, which
                           lives in its own repository: https://github.com/kuml-dev/kuml-vscode
...
kuml-tests/
  ...
  kuml-vault-examples-tests   ✅  47 CI smoke tests: render all vault examples incl. BPMN + ERM (V3.0.x+)
