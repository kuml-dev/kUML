...
| _Experimental spike (ADR-0011)._ Treat an Obsidian-vault-style directory of Markdown files with YAML frontmatter and embedded ` ```kuml ` blocks as an Open Knowledge Format (OKF) workspace: `info` summarises documents by type, `validate` checks frontmatter/structure, `render` renders the embedded diagrams to SVG.
