...
| `kuml workspace init\|info\|validate\|render`
| _Experimental spike (ADR-0011)._ Treat an Obsidian-vault-style directory of Markdown files with YAML frontmatter and embedded ` ```kuml ` blocks as an Open Knowledge Format (OKF) workspace: `init` scaffolds a new workspace (`--name`, `--mode knowledge\|engineering`, `--output`), `info <dir>` summarises documents by type, `validate <dir>` checks frontmatter/structure, `render <dir>` renders the embedded diagrams to SVG.
