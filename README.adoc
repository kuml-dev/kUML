...
| *Docker*
| `docker pull ghcr.io/kuml-dev/kuml-cli:<version>` — minimal CLI image (V2.0.38).

| *Native installers*
| DEB, RPM, DMG, MSI packages built by the `release-installers.yml` CI workflow on every version tag (V2.0.38). Download from the GitHub Releases page.

| *Maven Central* (library modules)
| `implementation("dev.kuml:kuml-core-dsl:0.7.0")` — library modules published to Maven Central.
|===
...
KerML, reverse engineering, XMI, Compose Desktop 🚧
| ✅ v0.7.0 released — V2.0.22–V2.0.38
|===
...
kuml-renderer/
  ...
  kuml-layout-grid   ✅  Pure-Kotlin grid layout engine (V1.1.12); default for Class/Component/UseCase
  kuml-kuiver        ✅  Compose Multiplatform renderer backend
  kuml-themes-core   ✅  Framework-neutral theme data (KumlColor, KumlTheme)
  kuml-themes        ✅  4 themes: Plain, kUML (navy/gold), Elegant, Playful — Compose adapter + SPI

kuml-io/
  kuml-io-svg        ✅  Direct SVG writer from LayoutResult (GraalVM-ready)
  kuml-io-png        ✅  SVG → PNG via Apache Batik 1.19 (Fat-JAR only)
  kuml-io-latex      ✅  LaTeX / TikZ export pipeline

kuml-cli/            ✅  Command-line interface
  render             ✅  kuml render model.kuml.kts [--format svg|png|latex] [--layout grid|elk|auto]
  watch              ✅  kuml watch model.kuml.kts  (live-reload, 500ms polling)
  validate           ✅  kuml validate model.kuml.kts --output text|json [--strict] [--no-check-structure]
  fmt                ✅  kuml fmt model.kuml.kts [--check]
  generate           ✅  kuml generate --plugin kotlin|java|sql model.kuml.kts
  completion         ✅  kuml completion bash|zsh|fish
  import             ✅  kuml import --format structurizr workspace.dsl
  export             ✅  kuml export --format structurizr model.kuml.kts
  ...
  transform          ✅  kuml transform model.kuml.kts --transformer <id> --output gen/
                         Built-in: uml-to-jpa, uml-to-rest, uml-to-k8s, uml-to-docker, c4-to-uml
  profile            ✅  kuml profile list|show|validate
  serve              ✅  kuml serve [--port N] [--host H]  (Web UI: CodeMirror 6 SPA + SVG/PNG/LaTeX)
  run                ✅  kuml run model.kuml.kts [--adapter stdin|mcp|batch] [--restore snap.json]
  update             ✅  kuml update check|notes  (GitHub Releases integration)

kuml-mcp/            ✅  MCP server — stdio JSON-RPC 2.0, Protocol 2024-11-05
                         Authoring tools (5): kuml.diagram.*, kuml.validate.*, ...
                         Runtime tools (5, V2.0.27): kuml.run.start / event / snapshot / patch / stop

kuml-runtime/        ✅  Behaviour runtime (V2.0.35)
...
kuml-web/            ✅  Browser-based editing environment (V2.0.34)
                         Ktor/Netty HTTP server + CodeMirror 6 SPA
                         REST API: POST /api/render (SVG/PNG/LaTeX), GET /api/themes, /api/examples
...
kuml-codegen/
  ...
  kuml-gen-java      ✅  Java code generator with JPA annotations from JavaEE stereotypes
  kuml-gen-sql       ✅  SQL DDL generator (CREATE TABLE + FK constraints from multiplicities)

kuml-gradle/         ✅  Gradle plugin (plugin ID: dev.kuml)
                         Tasks: kumlRender, kumlGenerate, kumlValidate (@CacheableTask, incremental)

kuml-jetbrains/      ✅  JetBrains IDE plugin (IntelliJ IDEA 2024.3+)
                         Annotator + 4 quick fixes, live SVG preview (Batik), structure view, code folding

kuml-vscode-plugin/  ✅  VS Code extension
                         TextMate grammar + DSL snippets + kUML: Render to SVG command
...
kuml-docs/
  ...
  kuml-asciidoc      ✅  AsciiDoc [kuml] block macro preprocessor (Antora-compatible)

kuml-packaging/      🚧  GraalVM Native Image infrastructure done; full native deferred (ADR-0009)
----
