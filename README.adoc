...
| *Docker*
| `docker pull ghcr.io/kuml-dev/kuml-cli:v0.16.0` — minimal CLI image.

| *Native installers*
| DEB, RPM, DMG, MSI packages built by the `release-installers.yml` CI workflow on every version tag (v0.16.0). Download from the GitHub Releases page.

| *Maven Central* (library modules)
| `implementation("dev.kuml:kuml-core-dsl:0.16.0")` — library modules published to Maven Central.
|===
