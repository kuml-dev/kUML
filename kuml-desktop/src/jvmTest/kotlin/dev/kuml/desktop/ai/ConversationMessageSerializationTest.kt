...
class ConversationMessageSerializationTest : FunSpec({
    val json = Json {
    ...
    test("Assistant message roundtrip with defaults") {
        val msg = ConversationMessage.Assistant(
            id = "a1",
            timestamp = 2000L,
            text = "I can help!",
            isStreaming = false,
    ...
    test("ToolCall message roundtrip") {
        val msg = ConversationMessage.ToolCall(
            id = "tc1",
            timestamp = 3000L,
            toolName = "add_class",
            argsJson = """{"name":"Foo"}""",
    ...
    test("ToolResult message roundtrip") {
        val msg = ConversationMessage.ToolResult(
            id = "tr1",
            timestamp = 4000L,
            toolCallId = "tc1",
            resultJson = """{"ok":true}""",
    ...
    test("ErrorMessage roundtrip") {
        val msg = ConversationMessage.ErrorMessage(
            id = "e1",
            timestamp = 5000L,
