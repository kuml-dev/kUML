...
| `stereotypes += "Name"`
| Cosmetic display-label stereotype (`«Name»` shown above the element, no tagged values,
  no profile required). Available on `classOf`, `attribute(...) { }` (block form), and
  `association(...) { }` — added for attributes and associations in V3.4 (ADR-0017:
  the DSL should expose everything the UML standard allows on the underlying metaclass,
  and `Property`/`Association` are just as stereotype-capable as `Class`).
...
| `associationClass(name, source: UmlClassifier, target: UmlClassifier, id?) { ... }`
| Both ends already have a builder `val` handle in scope (most common).

| `associationClass(name, sourceId: String, targetId: String, id?) { ... }`
| Referencing a classifier declared elsewhere by ID (e.g. across a larger script, or
  when reconstructing a diagram programmatically).

| `associationClass(name, source: UmlTypeRef, target: UmlTypeRef, id?) { ... }`
