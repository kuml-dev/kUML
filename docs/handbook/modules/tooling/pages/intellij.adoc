...
The zoom model was revised in v0.19.0: all fit/zoom actions control the preferred size of
the SVG canvas rather than Batik's internal rendering transform. Batik scales the SVG
automatically to the actual canvas size, so the visual result is identical to the previous
approach but fully compatible with the scroll pane.
