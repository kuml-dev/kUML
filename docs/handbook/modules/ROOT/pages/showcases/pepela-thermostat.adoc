...
their JSON payload. kUML's OCL expression language supports both `Integer` and
`Real` arithmetic with automatic promotion, but this example sticks to whole-degree
readings, so the 1 °C hysteresis band is expressed as:
