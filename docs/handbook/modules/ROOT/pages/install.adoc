...
https://chocolatey.org[Chocolatey] is the established package manager for Windows
developers and DevOps — the Windows counterpart to Homebrew. The kUML package ships
a self-contained Java 21 runtime (jlink), so **no separate JDK installation is required**.
...
moderation (typically 1–3 days); until it is approved, `choco install kuml` may not yet
resolve. Use xref:#_direct_download[a direct download] in the meantime. Subsequent version
updates are approved automatically.
====

=== SDKMAN! (coming soon)

SDKMAN! is the JVM-ecosystem package manager (`kotlin`, `gradle`, `maven`, `springboot`, …).
kUML is published as a `PLATFORM_SPECIFIC` candidate with jlink-bundled JREs per platform —
...
xref:#_homebrew_macos_linux[Homebrew], the native installers, or a
xref:#_direct_download[direct download]. This page will be updated with the working
`sdk install kuml` command as soon as the candidate goes live.
====
...
Planned platforms: Apple Silicon (`MAC_ARM64`), Intel Mac (`MAC_OSX`), Linux x64 (`LINUX_64`).
Windows support is on the V2.x roadmap.
...
[WARNING]
====
The `dev.kuml` Gradle plugin is **not yet published** to the Gradle Plugin Portal or
Maven Central. The example below is the target usage once it ships — until then, use a
local `publishToMavenLocal` build; see the
xref:tooling:gradle.adoc#_publishing_the_plugin_from_your_own_fork[Gradle plugin page]
for the exact workaround steps.
====
