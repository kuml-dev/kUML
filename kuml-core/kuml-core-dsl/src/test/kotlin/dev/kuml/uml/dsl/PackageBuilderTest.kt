...
class PackageBuilderTest : FunSpec({
    ...
    test("enumOf inside package gets qualified id") {
        val pkg =
            umlModel("M") {
                `package`("domain") {
                    enumOf("OrderStatus") { literal("DRAFT") }
                }
            }.elements.filterIsInstance<UmlPackage>().first()
