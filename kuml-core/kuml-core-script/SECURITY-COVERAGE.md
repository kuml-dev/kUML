...
1. **Linux-OS-Käfig (Welle 5) ist verhaltens-ungetestet.** Die `bwrap`-Argv-
   Konstruktion ist korrekt und unit-verifiziert (`OsSandboxTest`), aber diese
   Entwicklungsmaschine ist macOS — es gibt keinen Linux-Kernel, auf dem die
   Namespace-Isolation tatsächlich ausgeübt werden könnte. Braucht echtes
   Linux-CI vor Produktionsvertrauen (konkrete Schritte: siehe
   Umsetzungsstand Welle 5, Punkt "Was für echte Linux-CI-Verifikation nötig
   ist").
