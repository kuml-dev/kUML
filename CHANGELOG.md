...
> as three separate versions (0.38.0/0.39.0/0.40.0) in quick succession. Consolidated
> into a single 0.38.0 release since none of the intermediate versions were tagged,
> published, or announced externally — no reason to ship three point releases for one
> afternoon's worth of related work.

### Added

**"Powered by kUML" attribution comment + opt-in watermark**

Two branding channels, both implemented once in `SvgDocument.render` (the single choke
point every `toSvg`/`toSvgFile` overload funnels through), so they automatically reach
every surface: CLI, Gradle plugin, MCP, and the Markdown/AsciiDoc doc processors.

An always-on metadata comment — `<!-- Generated by kUML v0.38.0 — kuml.dev -->` — is
now emitted on every rendered SVG, placed after the optional `<?xml` declaration.
It's independent of the existing `embedThemeAsComment` flag (which only gates the
separate theme/engine debug comment) and independent of the watermark below. The
version comes from a build-time-generated Kotlin constant (`KumlBuildInfo.VERSION`, a
new `generateKumlBuildInfo` Gradle task) rather than a hardcoded literal, since
`kuml-io-svg` is a Kotlin Multiplatform module (jvm/js/wasmJs) and the existing
`kuml-cli` `KumlVersion` (a JVM `.properties` lookup) isn't reachable from
`commonMain`. Blueprint diagrams — the one renderer that bypasses
`SvgDocument.render` entirely (deterministic grid geometry, no `LayoutResult`) — call
the extracted `SvgDocument.attributionComment()` helper directly so the guarantee
still holds there, prepending it before the `<svg>` root since Blueprint emits no XML
declaration.

An opt-in visible "Powered by kUML" watermark (`SvgRenderOptions.watermark`, default
`false`) grows the canvas downward by a small band below the existing bottom padding
— so it structurally cannot overlap diagram content — and draws a right-aligned label
themed via the theme's `typography.small`/`colors.muted` roles, plus a narrow-canvas
guard that widens the canvas when a diagram is narrower than the label itself. Wired
end-to-end: a new `--watermark` flag on `kuml render` (CLI), a new
`kuml { watermark.set(true) }` extension property on the Gradle plugin
(`@Input` on `KumlRenderTask` for correct UP-TO-DATE invalidation), and a new optional
`svgOptions` parameter on `KumlPngRenderer`'s `toPng(...)` overloads so the watermark
reaches PNG rasterization too (previously always `SvgRenderOptions.DEFAULT`
internally). MCP needed no change — it already uses `SvgRenderOptions.DEFAULT`, so the
watermark stays off there by construction. Blueprint is the one diagram type where
`--watermark`/`watermark.set(true)` has no effect, since its `toSvg(BlueprintModel,
BlueprintDiagram, ...)` overload takes no `SvgRenderOptions`; the always-on
attribution comment above is unaffected by this gap.
...
**Blueprint sample-output PNGs stopped generating after the attribution comment landed**

The renderer/layout PNG-inspection routine's `SampleOutput.write` helper only
side-rendered a PNG when the SVG content started with `<svg` or `<?xml`. Once the
always-on attribution comment made every Blueprint SVG start with `<!-- ... -->`
instead (Blueprint emits no XML declaration), Blueprint sample-output PNGs silently
stopped being written. Both `SampleOutput` copies (`kuml-io-svg` jvmTest and
`kuml-vault-examples-tests`) now also accept a leading XML comment that wraps an
`<svg>` root.

### Security

**Defense-in-depth: escape theme-derived strings in the watermark's `<text>` element**

The watermark interpolated the theme's `typography.small.family` and
`colors.muted.toHex()` raw into the `<text>` element, and dropped the watermark label
raw as element content — bypassing the renderer-wide `xmlEscapeAttr`/`xmlEscapeContent`
convention every other node/text emitter in `kuml-io-svg` follows. Not exploitable
today (the hex fill is structurally `#RRGGBB`, the label is a compile-time literal,
and a config script can only select a theme by registry name, not supply an arbitrary
font-family string), but the family slot is a free-form `String`, so raw
interpolation was a latent injection vector against any future feature that sources
theme strings from less-trusted input. Fixed by routing both attributes and the label
through the existing escape utilities; output is byte-identical for all built-in
themes.
