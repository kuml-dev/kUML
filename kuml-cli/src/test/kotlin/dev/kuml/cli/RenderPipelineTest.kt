...
class RenderPipelineTest :
    FunSpec({
        ...
        test("RenderPipeline writes SVG file from Blueprint/Journey-Map script") {
            ...
            val content = outputFile.toFile().readText()
            // "Powered by kUML" branding — the always-on attribution comment is
            // prepended before the root <svg> tag (Blueprint bypasses
            // SvgDocument.render's own XML-declaration/comment assembly; see
            // BlueprintGridSvg.renderBlueprintJourney).
            content shouldStartWith "<!-- Generated by kUML v"
            content shouldContain "<svg"
        ...
        // ── "Powered by kUML" branding (default-off watermark + always-on comment) ──

        test("RenderPipeline embeds the always-on attribution comment regardless of --watermark") {
            val fixture = File("src/test/resources/minimal.kuml.kts")
            val outputDir = Files.createTempDirectory("kuml-branding-attribution-test")
            val outputFile = outputDir.resolve("minimal.svg")
            ...
            val content = outputFile.toFile().readText()
            content shouldContain "<!-- Generated by kUML v"
            content shouldContain "— kuml.dev -->"

            outputFile.toFile().delete()
            outputDir.toFile().delete()
        }

        test("RenderPipeline does not render the visible watermark by default") {
            val fixture = File("src/test/resources/minimal.kuml.kts")
            val outputDir = Files.createTempDirectory("kuml-branding-default-off-test")
            val outputFile = outputDir.resolve("minimal.svg")
            ...
            val content = outputFile.toFile().readText()
            content shouldNotContain "Powered by kUML"

            outputFile.toFile().delete()
            outputDir.toFile().delete()
        }

        test("RenderPipeline renders the visible watermark when watermark=true (SVG)") {
            val fixture = File("src/test/resources/minimal.kuml.kts")
            val outputDir = Files.createTempDirectory("kuml-branding-watermark-svg-test")
            val outputFile = outputDir.resolve("minimal.svg")
            ...
            val content = outputFile.toFile().readText()
            content shouldContain "Powered by kUML"
            content shouldContain """text-anchor="end""""

            outputFile.toFile().delete()
            outputDir.toFile().delete()
        }

        test("RenderPipeline renders the visible watermark when watermark=true (PNG)") {
            val fixture = File("src/test/resources/minimal.kuml.kts")
            val outputDir = Files.createTempDirectory("kuml-branding-watermark-png-test")
            val outputFile = outputDir.resolve("minimal.png")
        ...
        test("RenderPipeline watermark=true also works for a C4 diagram (svg)") {
            val fixture = File("src/test/resources/minimal-c4.kuml.kts")
            val outputDir = Files.createTempDirectory("kuml-branding-watermark-c4-test")
            val outputFile = outputDir.resolve("minimal-c4.svg")
