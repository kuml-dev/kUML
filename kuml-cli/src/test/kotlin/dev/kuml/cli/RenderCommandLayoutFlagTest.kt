...
class RenderCommandLayoutFlagTest :
    FunSpec({
        ...
        test("Registry: grid registered first → pickFor(UmlClass, null) returns kuml.grid") {
            LayoutEngineRegistry.clear()
        ...
        test("Registry: explicit elk.layered preference for UmlClass → returns elk") {
            LayoutEngineRegistry.clear()
            LayoutEngineRegistry.register(GridLayoutEngineProvider())
            LayoutEngineRegistry.register(ElkLayoutEngineProvider())
        ...
        test("kuml render minimal.kuml.kts (no --layout flag) → SVG produced with grid engine") {
            ...
            // Default: no layout override → auto → grid for class diagram
            RenderPipeline.run(
                input = fixture,
                output = outputFile,
                format = "svg",
