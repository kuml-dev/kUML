...
class ErmContentSizeProviderTest :
    FunSpec({
        ...
        val baseline = ErmEntity(id = "base", name = "Base", attributes = listOf(ErmAttribute(id = "b1", name = "id", type = ErmDataType.Uuid, primaryKey = true)))
        val hub =
            ErmEntity(
                id = "hub",
                name = "Hub",
        ...
        val spoke1 = ErmEntity(id = "s1", name = "S1", attributes = listOf(ErmAttribute(id = "s1id", name = "id", type = ErmDataType.Uuid, primaryKey = true)))
        val spoke2 = ErmEntity(id = "s2", name = "S2", attributes = listOf(ErmAttribute(id = "s2id", name = "id", type = ErmDataType.Uuid, primaryKey = true)))
        val spoke3 = ErmEntity(id = "s3", name = "S3", attributes = listOf(ErmAttribute(id = "s3id", name = "id", type = ErmDataType.Uuid, primaryKey = true)))
        ...
        test("connection buffer is capped at CONNECTION_PUFFER_MAX_PX") {
            val spokes = (1..30).map { ErmEntity(id = "spoke$it", name = "Spoke$it", attributes = listOf(ErmAttribute(id = "spoke${it}id", name = "id", type = ErmDataType.Uuid, primaryKey = true))) }
        ...
        test("more attribute rows grow the box height") {
            val small = ErmEntity(id = "small", name = "Small", attributes = listOf(ErmAttribute(id = "sm1", name = "id", type = ErmDataType.Uuid, primaryKey = true)))
            val many =
                ErmEntity(
                    id = "many",
                    name = "Many",
        ...
        test("real ELK run: content-aware sizing is only visible through the bridge, not a hardcoded LayoutResult") {
            ...
            val hubBounds = layout.nodes.entries.first { it.key.value == "hub" }.value.bounds
            val baseBounds = layout.nodes.entries.first { it.key.value == "base" }.value.bounds
