...
public object LayoutEngineRegistry {
    ...
    public fun loadFromClasspath() {
        loadProvidersFromClasspath().forEach { register(it) }
    }
...
/**
 * Plattform-spezifische Classpath-Discovery für [KumlLayoutEngineProvider]s.
 * JVM nutzt [java.util.ServiceLoader]; js/wasmJs liefern eine leere Liste, da
 * es dort keine Classpath-SPI gibt — Provider werden dort explizit via
 * [LayoutEngineRegistry.register] registriert.
 */
internal expect fun loadProvidersFromClasspath(): List<KumlLayoutEngineProvider>
