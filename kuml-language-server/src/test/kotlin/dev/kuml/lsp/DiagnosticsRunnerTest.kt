...
class DiagnosticsRunnerTest :
    FunSpec({
        ...
        test("happy path: parses a single TSV diagnostic emitted by the CLI") {
            ...
            try {
                DiagnosticsRunner.run(text = "class Foo", cliPath = stub, timeoutMs = 30_000) shouldBe
                    listOf(
                        KumlDiagnostic(
        ...
        test("valid file: stub emits nothing, run returns an empty list") {
            ...
            try {
                DiagnosticsRunner.run(text = "class Foo", cliPath = stub, timeoutMs = 30_000).shouldBeEmpty()
            } finally {
                stub.parentFile.deleteRecursively()
            }
        }
        ...
        test("bogus CLI path returns an empty list and never throws") {
            DiagnosticsRunner.run(text = "class Foo", cliPath = File("/no/such/kuml"), timeoutMs = 30_000).shouldBeEmpty()
        }

        test("timeout: destroys the process and returns empty within a couple seconds") {
            val stub = FakeCli.write(tsvLines = emptyList(), sleepMs = 5_000) ?: return@test
            try {
                val start = System.currentTimeMillis()
                val result = DiagnosticsRunner.run(text = "class Foo", cliPath = stub, timeoutMs = 200)
