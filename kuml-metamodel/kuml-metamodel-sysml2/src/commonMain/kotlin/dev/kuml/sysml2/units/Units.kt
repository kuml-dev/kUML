...
data class Unit(
    ...
    companion object {
        ...
        val KILOGRAM: Unit = Unit(symbol = "kg", longName = "kilogram", quantityKind = "Mass")

        /** Length — m. */
        val METRE: Unit = Unit(symbol = "m", longName = "metre", quantityKind = "Length")

        /** Time — s. */
        val SECOND: Unit = Unit(symbol = "s", longName = "second", quantityKind = "Time")

        /** Speed — km/h, the automotive-friendly form (m/s is V2.x). */
        val KMPH: Unit = Unit(symbol = "km/h", longName = "kilometres per hour", quantityKind = "Speed")

        /** Power — kW. */
        val KILOWATT: Unit = Unit(symbol = "kW", longName = "kilowatt", quantityKind = "Power")

        /** Energy — kWh, the battery-pack-friendly unit. */
        val KILOWATT_HOUR: Unit = Unit(symbol = "kWh", longName = "kilowatt-hour", quantityKind = "Energy")

        /** Electric potential — V. */
        val VOLT: Unit = Unit(symbol = "V", longName = "volt", quantityKind = "Voltage")

        /** Electric current — A. */
        val AMPERE: Unit = Unit(symbol = "A", longName = "ampere", quantityKind = "Current")

        /** Plane angle — deg (rad as a peer in V2.x). */
        val DEGREE: Unit = Unit(symbol = "deg", longName = "degree", quantityKind = "Angle")
 ...
 */
val Number.kg: UnitValue get() = UnitValue(value = toDouble(), unit = Unit.KILOGRAM)
val Number.m: UnitValue get() = UnitValue(value = toDouble(), unit = Unit.METRE)
val Number.s: UnitValue get() = UnitValue(value = toDouble(), unit = Unit.SECOND)
val Number.kmph: UnitValue get() = UnitValue(value = toDouble(), unit = Unit.KMPH)
val Number.kW: UnitValue get() = UnitValue(value = toDouble(), unit = Unit.KILOWATT)
val Number.kWh: UnitValue get() = UnitValue(value = toDouble(), unit = Unit.KILOWATT_HOUR)
val Number.V: UnitValue get() = UnitValue(value = toDouble(), unit = Unit.VOLT)
val Number.A: UnitValue get() = UnitValue(value = toDouble(), unit = Unit.AMPERE)
val Number.deg: UnitValue get() = UnitValue(value = toDouble(), unit = Unit.DEGREE)
